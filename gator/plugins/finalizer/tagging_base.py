...
"""
gator.plugins.finalizer.tagging_base
=====================================
...
from gator.exceptions import FinalizerException
from gator.plugins.finalizer.base import BaseFinalizerPlugin
...
class TaggingBaseFinalizerPlugin(BaseFinalizerPlugin):
    ...
    def add_plugin_args(self):
        ...
        creator_help = 'The user who is gatoring. The resultant AMI will receive a creator tag w/ this user'
    ...
    def _set_metadata(self):
        ...
        creator = context.ami.get('creator', config.get('creator', 'gator'))
    ...
    def __enter__(self):
        ...
        if context.ami.get("suffix", None):
            environ["GATOR_AMI_SUFFIX"] = context.ami.suffix
        if context.ami.get("creator", None):
            environ["GATOR_CREATOR"] = context.ami.creator
        if context.ami.get("vm_type", None):
            environ["GATOR_VM_TYPE"] = context.ami.vm_type
        if context.ami.get("enhanced_networking", None):
            environ["GATOR_ENHANCED_NETWORKING"] = str(int(context.ami.enhanced_networking))
        if context.ami.get("ena_networking", None):
            environ["GATOR_ENA_NETWORKING"] = str(int(context.ami.ena_networking))
